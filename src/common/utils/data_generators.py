...
def generate_categories(n=5):
    category_names = [
        'Електроніка', 'Одяг', 'Іграшки', 'Канцелярія', 'Продукти'
    ]
    categories = []
    for i in range(n):
    ...
    for _ in range(n):
        ...
        for attempt in range(10):
            ...
            if not barcode or barcode == '':
                print(f'УВАГА: штрихкод порожній на спробі {attempt+1}, генеруємо вручну')
                barcode = str(random.randint(10**12, 10**13-1))
            if not Product.objects.filter(barcode=barcode).exists():
                break
            else:
                print(f'УВАГА: штрихкод {barcode} вже існує в БД, повторна спроба')
                barcode = ''
        if not barcode:
...
def generate_operations(n=100, products=None, user=None):
    ...
    operations = []
    operation_types = [OPERATION_CHOICES.RECEIPT, OPERATION_CHOICES.ISSUE, OPERATION_CHOICES.WRITE_OFF]
    for _ in range(n):

...
from wms.models import OPERATION_CHOICES, Product, StockOperation
...
def process_stock_operation_view(request, operation_type, increase=True):
    if request.method == "POST":
        ...
        success, message, operation = process_stock_operation(
            cart, request.user, operation_type, reason, note, notification_type=operation_type, increase=increase
        )
        if success:
            return JsonResponse(
                {
...
def get_dashboard_context():
    ...
    from wms.utils import (get_categories_with_stats, get_low_stock_products,
...
def handle_operation_cart(request, operation_type, cart_key, success_message, error_message):
    ...
    if "add_product" in request.POST:
        ...
        success, message, operation = process_stock_operation(
            ...
            reason,
            note,
            notification_type=operation_type,
            increase=(operation_type == OPERATION_CHOICES.RECEIPT),
        )
        if success:
            request.session[cart_key] = []
